...
mod hide {
    use std::str::FromStr;
    pub trait TweakableValue: 'static + FromStr + Clone + Send {}
    impl<T: 'static + FromStr + Clone + Send> TweakableValue for T {}
}

use hide::TweakableValue;
...
lazy_static! {
    static ref VALUES: Mutex<HashMap<(&'static str, u32), TweakValue>> = Mutex::new(HashMap::new());
}
...
fn get_value<T: TweakableValue>(default: T, file: &'static str, line: u32) -> Option<T> {
...
pub fn inline_tweak<T: TweakableValue>(default: T, file: &'static str, line: u32) -> T {
    if cfg!(debug_assertions) {
        get_value(default.clone(), file, line).unwrap_or_else(move || default)
    } else {
        default
    }
}
